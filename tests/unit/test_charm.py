...
def test_write_ceph_cli_config(ceph_data, harness):
...
@mock.patch("charm.CephCsiCharm.ceph_data", new_callable=mock.PropertyMock)
def test_write_ceph_cli_keyring(ceph_data, harness):
...
@mock.patch("charm.CephCsiCharm.ceph_data", new_callable=mock.PropertyMock)
def test_ceph_context_getter(ceph_data, check_output, harness):
    ...
    check_output.assert_any_call(["/usr/bin/ceph", "--user", "ceph-csi", "fsid"], timeout=60)
    check_output.assert_any_call(
        ["/usr/bin/ceph", "--user", "ceph-csi", "fs", "ls", "-f", "json"], timeout=60
    )
